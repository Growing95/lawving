...
<body>
	...
	<table align="center" width="850" border="1" cellspacing="0"
		...
		<tr>
			<th>날 짜</th>
			<td><fmt:formatDate value="${notice.notice_reg }"
					pattern="yyyy-MM-dd" /></td>
		</tr>
		<tr>
			<th>첨부파일</th>
			<td><c:if test="${ !empty notice.notice_file_name }">
					<%-- 첨부파일이 있다면 다운로드 설정함 --%>
					<c:url var="unf" value="/download_notice.do">
						<c:param name="download_notice"
							value="${ notice.notice_file_name }" />
					</c:url>
					<a href="${ unf }">${notice.notice_file_name }</a>
				</c:if> <c:if test="${ empty notice.notice_file_name }">&nbsp;</c:if></td>
		</tr>
		<tr>
			<th>내 용</th>
